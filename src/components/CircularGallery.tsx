...
class App {
  ...
  onCheck() {
    // Snapping to the nearest item would overwrite the drift's target every
    // time the wheel settles, which reads as the gallery stopping dead.
  ...
  update() {
    // Drifts on its own; a drag takes over while the pointer is down.
    if (this.autoScroll && !this.isDown) this.scroll.target += this.autoScroll;
...
interface CircularGalleryProps {
  ...
  scrollSpeed?: number;
  scrollEase?: number;
  /** Units of drift per frame; 0 disables. */
  autoScroll?: number;
}
